                ...
                if i + col_idx < len(day_matches):
                    ...
                    with col:
                        with st.container():
                            ...
                            # Если нужен ручной ввод, добавляем поля под карточкой
                            if row['manual_input_needed']:
                                st.markdown("---")
                                st.caption("Введите коэффициенты вручную (изменения применяются автоматически):")
                                # Поля уже созданы выше внутри цикла, но они были созданы в основном цикле, а не здесь.
                                # Чтобы избежать дублирования, мы не можем повторно создать поля здесь.
                                # Вместо этого мы можем оставить поля в основном цикле, но они будут над карточками.
                                # Лучше разместить поля прямо под карточкой, но для этого нужно перенести создание полей в этот блок.
                                # Однако это сложно, потому что поля должны быть привязаны к конкретному матчу.
                                # Альтернатива: создать поля внутри этого блока с уникальными ключами.
                                # Мы можем сделать так: в основном цикле мы создаём поля и сохраняем их значения в сессию, а здесь просто отображаем их (но они уже отображены).
                                # Проблема в том, что поля создаются в основном цикле, который выполняется до этого блока, поэтому они будут отображаться над карточками, а не под ними.
                                # Чтобы разместить поля под карточкой, нужно создать их внутри этого блока.
                                # Поэтому я перепишу логику: в основном цикле мы будем только вычислять вероятности и сохранять данные, а отображение карточек и полей сделаем здесь.
                                # Это потребует перестройки кода, но я сейчас дам финальную версию с правильным расположением.
